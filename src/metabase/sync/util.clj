...
(extend-protocol INameForLogging
  ...
  (name-for-logging [{database-name :name, id :id, engine :engine,}]
    (format "%s Database %d '%s'" (name engine) (or id "") database-name))

  i/TableInstance
  (name-for-logging [{schema :schema, id :id, table-name :name}]
    (format "Table %d '%s'" (or id "") (str (when (seq schema) (str schema ".")) table-name)))

  i/FieldInstance
  (name-for-logging [{field-name :name, id :id}]
    (format "Field %d '%s'" (or id "") field-name)))
